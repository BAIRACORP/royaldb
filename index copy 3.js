...
app.put('/api/trips/:id/complete', async (req, res) => {
  ...
  if ([startMeter, endMeter, finalBill].some(v => v == null)) {
    return res.status(400).json({ message: 'startMeter, endMeter and finalBill are required' });
  }

  try {
    const [result] = await db.query(
      `UPDATE trips SET
         startMeter = ?, endMeter = ?, luggage = ?, pet = ?, toll = ?,
         hills = ?, totalKm = ?, finalKm = ?, finalBill = ?,
         status = 'completed', completed_at = NOW()
       WHERE id = ?`,
      [startMeter, endMeter, luggage, pet, toll, hills, totalKm, finalKm, finalBill, tripId]
    );
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: 'Trip not found' });
    }
    res.json({ message: 'Trip completed', tripId });
  } catch (err) {
    console.error(err);
...
app.post('/api/bills', async (req, res) => {
  const d = req.body;
  console.log('🌀 Incoming bill data:', JSON.stringify(d));

  if (!d.driverEmail || !d.customerName || d.finalBill == null) {
    console.warn('❌ Missing required fields in bill data:', d);
    return res.status(400).json({ message: 'driverEmail, customerName, finalBill are required' });
  }

  try {
    const fields = [
      d.driverEmail, d.customerName, d.phone || null,
      d.pickupLocation || null, d.dropLocation || null,
      d.pickupDate || null, d.pickupTime || null, d.tripType || null,
      d.startMeter || 0, d.endMeter || 0,
      d.totalKm || 0, d.finalKm || 0,
      d.kmPrice || 0, d.totalKmPrice || 0,
      d.luggageCharge || 0, d.petCharge || 0,
      d.tollCharge || 0, d.hillsCharge || 0,
      d.bettaCharge || 0, d.stateCharge || 0,
      d.totalEnteredCharges || 0, d.finalBill,
    ...
    const [result] = await db.query(
      `INSERT INTO bills (
        ...
        pickupLocation, dropLocation,
        pickupDate, pickupTime, tripType,
        startMeter, endMeter, totalKm, finalKm,
        kmPrice, totalKmPrice,
        luggageCharge, petCharge, tollCharge, hillsCharge,
        bettaCharge, stateCharge,
        totalEnteredCharges, finalBill, createdAt
      ) VALUES (${placeholders})`,
      fields
    );
    ...
    console.error('❌ Error on INSERT bills:', err);
